...
<div class="d-flex justify-content-around mb-4">
        ...
        <form action="" method="POST" class="bg-light p-2 rounded" id="formUserRegister">
            <div class="mb-4 mt-4">
                <div class="form-floating mb-3">
                    <input type="number" class="form-control form-date" id="floatingIdentity" placeholder="0000"
                        required name="user_show">
                    <label for="floatingIdentity">Identifiant*</label>
                </div>
                <p class="mb-1">*Obligatoire</p>
                <div class="d-flex justify-content-center">
                    <button type="submit" class="btn btn-warning form-bouton" name="show_user"
                        id="show_user_register">Afficher</button>
                </div>
            </div>
        </form>
  ...
  echo('<p class=text-center>Cette identifiant n\'est pas enregistré</p>');
    ...
    if(isset($_POST[" show_user"])){ foreach($resultats as $resultat){
        ...
        <select class="form-select form-date mb-2 mt-2" aria-label="select" name="id_poste" required>
            ...
            <option value="<?= htmlspecialchars($freeComputer->id_postes); ?>">Poste n°
                <?= htmlspecialchars($freeComputer->numero_poste); ?>
            </option>
            <?php endforeach; ?>
        </select>
        <!-- bouton pour reserver -->
        <button type="submit" class="btn btn-warning form-bouton" name="booking_choose"
            id="btnBookingChoose">Reserver</button>
            ...
            <tbody>
                ...
                <tr>
                    <td scope="row" class="text-center">
                        <?= htmlspecialchars($resultat->nom_utilisateur); ?>
                    </td>
                    <td class="text-center">
                        <?= htmlspecialchars($resultat->prenom_utilisateur); ?>
                    </td>
                    <td class="text-center">
                        <?= htmlspecialchars($resultat->numero_utilisateur); ?>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php endif;?>
...
<div class="d-flex justify-content-around mb-4">
    <form action="" method="POST" class="bg-light p-2 rounded" id="formDateBooking">
        <div class="form-floating mb-3">
            <input type="date" class="form-control form-date" id="floatingDateUser" placeholder="00/00/0000"
        ...
        <!-- bouton pour reserver -->
        <button type="submit" class="btn btn-warning form-bouton" name="booking_show_register"
            id="showBooking">Afficher</button>
    ...
    <div>
        <form action="" method="POST" class="ms-5 bg-light p-2 rounded" id="formIdentitySelect">
            <div class="mb-4 mt-4">
                <div class="form-floating mb-3">
                    <input type="number" class="form-control form-date" id="floatingIdentityUser" placeholder="0000"
                        required name="select_identity_user">
                    <label for="floatingIdentityUser">Identifiant*</label>
                </div>
                <p class="mb-1">*Obligatoire</p>

                <button type="submit" class="btn btn-warning form-bouton" name="select_identity"
                    id="showRegisterBtn">Afficher</button>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center">
                    <?= htmlspecialchars($user->prenom_utilisateur); ?>
                </td>
                <td class="text-center">
                ...
                <td class="text-center">
                    <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#<?= "
                        deleteBookingrModal_". htmlspecialchars($user->id_creneaux);?>"><svg
